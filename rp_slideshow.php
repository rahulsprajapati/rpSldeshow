...
<div class="rp_slideshow_admin rp_container">
  ...
  <span><strong>Note : </strong>Slide Image Minimum Size => width = 640px and height = 340px</span>
		<hr />
	<form action="" id="form1" class="rp_left" method="POST" name="media_upload" enctype="multipart/form-data">
	  	<input type="hidden" name="action" id="action" value="rp_ajax_upload"/>
		Slide Name : <input type="text" class="rp_text_box" name="title_name" /> 
	  	<input type="button" name="UploadImage" id="uploadImage" class="rp_btn"	value="Upload Image" />
	  	<span id="imageSrc">No Image Is Selected</span>
	  	<input type="file" name="url" id="slide1" />
		<input type="submit" name="Upload" id="upload" class="rp_btn"	value="Add Slide" />
	</form>	
  	<input type="button" name="change_order" id="change_order" class="rp_btn rp_right"	value="Change Slide Order" />
	<input type="button" name="save" id="save" class="rp_btn rp_right"	value="Save Slideshow" />
...
function all_scripts(){
  ...
  wp_enqueue_script('rpSlideshow_jquery',plugin_dir_url(__FILE__).'js/jquery-1.10.2.js',array('jquery'), '1.0.0', false);
  wp_enqueue_script('rpSlideshow_jquery_ui',plugin_dir_url(__FILE__).'js/jquery-ui.js',array('rpSlideshow_jquery'), '1.0.0', false);
  wp_enqueue_script('rpSlideshow_script',plugin_dir_url(__FILE__).'js/script.js',array('rpSlideshow_jquery_ui'), '1.0.0', false); 
  wp_enqueue_script('rpSlideshow_ajax_script',plugin_dir_url(__FILE__).'js/ajax_script.js',array('rpSlideshow_script'), '1.0.0', true); 
}
  ...
  if($order != ""){
	...
	for($i=0;$i< count($order) ;$i++){
	  ...
	  ?>
		...
		<div class="row rp_image_row rp_inline_block" id='<?php echo "ID_".$order[$i]; ?>'>
		  
			<label class="rp_left rp_slide_label">Slide<?php echo $i+1;?></label>
			<input type="button" onclick="deleteImage(<?php echo $order[$i];?>)" name="delete_image" class="rp_btn rp_delete_btn" value="X" />
			<div class="rp_clear"></div>
			<hr class="rp_hr" />
			<div class="column rp_block">
				<div class="wrap">
					...
					<div class="img_wrap">
						<img class="normal" src="<?php echo $rpslideshow_post->post_content;?>" />
					</div>
				</div>
			</div>
...
function rp_Slideshow_Shortcode(){
  ...
  wp_enqueue_script('rpSlideshow_bootstrap_jq_script',plugin_dir_url(__FILE__).'js/jquery.min.js',array('rpSlideshow_script'), '1.0.0', false);
  wp_enqueue_script('rpSlideshow_bootstrap_script',plugin_dir_url(__FILE__).'js/bootstrap.min.js',array('rpSlideshow_bootstrap_jq_script'), '1.0.0', false);
